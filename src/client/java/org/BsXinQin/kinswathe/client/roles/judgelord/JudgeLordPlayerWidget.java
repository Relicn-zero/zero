package org.BsXinQin.kinswathe.client.roles.judgelord;
...
public class JudgeLordPlayerWidget extends ButtonWidget {

    public final LimitedInventoryScreen screen;
    public final UUID targetUUID;
    public final PlayerListEntry targetPlayerEntry;

    public JudgeLordPlayerWidget(@NotNull LimitedInventoryScreen screen, int x, int y, @NotNull UUID targetUUID, @NotNull PlayerListEntry targetPlayerEntry) {
        ...
        if (canUse) {
            ...
            if (this.isHovered()) {
                ...
                context.drawTooltip(MinecraftClient.getInstance().textRenderer, Arrays.asList(
                    ...
                    Text.translatable("hud.kinswathe.judgelord.ready").copy().withColor(Color.GREEN.getRGB())),
                    this.getX() - 4 - MinecraftClient.getInstance().textRenderer.getWidth(targetPlayerEntry.getProfile().getName()) / 2,
                    this.getY() - 19
                );
            }
        } else {
            ...
            if (this.isHovered()) {
                ...
                if (ability.cooldown > 0) {
                    ...
                    context.drawTooltip(MinecraftClient.getInstance().textRenderer, Arrays.asList(
                        Text.of(targetPlayerEntry.getProfile().getName()),
                        Text.translatable("tip.kinswathe.ability.not_enough_money", ConfigWorldComponent.KEY.get(MinecraftClient.getInstance().player.getWorld()).JudgeLordAbilityPrice)),
                        this.getX() - 4 - MinecraftClient.getInstance().textRenderer.getWidth(targetPlayerEntry.getProfile().getName()) / 2,
                        this.getY() - 19
                    );
                }
