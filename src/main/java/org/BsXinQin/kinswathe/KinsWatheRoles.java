...
public class KinsWatheRoles {
    ...
    // 裁决者
    public static Role ARBITER = registerRole(new Role(
    ...
    public static void setDefaultEvents() {
        ModdedRoleAssigned.EVENT.register((player, role)->{
            ...
            if (role.equals(ARBITER)) {
                // 不给初始物品（留空），或者给一个自定义物品
                // player.giveItemStack(SomeItem.getDefaultStack());
            }
        });
    }
    ...
    public static void registerRolesAbility() {
        ...
        // 裁决者能力注册
        ServerPlayNetworking.registerGlobalReceiver(ArbiterC2SPacket.ID, (payload, context) -> {
